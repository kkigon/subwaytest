...
const expectedStationCounts = { DG1: 35, DG2: 29, DG3: 30, DGK: 8, DJ1: 22, GJ1: 20 };
...
assert.ok(daeguStation("사곡").x < daeguStation("북삼").x, "북삼역은 사곡역 다음에 있어야 한다");
assert.ok(daeguStation("북삼").x < daeguStation("왜관").x, "북삼역은 왜관역 전에 있어야 한다");
assert.equal(daeguStation("북삼").y, daeguStation("왜관").y, "북삼역은 대경선 서쪽 수평축에 있어야 한다");
...
assert.match(html, /<script src="js\/data-nationwide\.js(?:\?v=[^"]+)?"><\/script>/);
